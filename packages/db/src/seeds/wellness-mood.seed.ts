...
import { generateWellnessMoodId } from '@peernest/core/utils/id-generator';
...
async function main() {
  ...
  const filename = 'wellness_mood_dataset';
  const fileExt = 'csv';
  ...
  const headers = ['wellness_mood_id', 'wellness_mood_name'];
  ...
  const appendedColumns = ['wellness_mood_position'];
  const formattedDataObj = dataObj.map((wellnessMood, idx) => {
    return {
      ...wellnessMood,
      wellness_mood_id: generateWellnessMoodId(),
      wellness_mood_position: idx,
    };
  });
