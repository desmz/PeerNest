...
import { generateWellnessFactorCategoryId } from '@peernest/core/utils/id-generator';
...
async function main() {
  ...
  const filename = 'wellness_factor_category_dataset';
  const fileExt = 'csv';
  ...
  const headers = ['wellness_factor_category_id', 'wellness_factor_category_name'];
  ...
  const appendedColumns = ['wellness_factor_category_position'];
  const formattedDataObj = dataObj.map((wellnessFactorCategory, idx) => {
    return {
      ...wellnessFactorCategory,
      wellness_factor_category_id: generateWellnessFactorCategoryId(),
      wellness_factor_category_position: idx,
    };
  });
