...
export async function up(db: TKyselyDB): Promise<void> {
  await sql`
    ...
    RETURNS trigger AS $$
    BEGIN
      NEW.user_display_name_tsv := to_tsvector('english', f_unaccent(coalesce(NEW.user_display_name, '')));
      RETURN NEW;
    END
    $$ LANGUAGE plpgsql;
  `.execute(db);
  ...
  await sql`
    CREATE INDEX user_display_name_tsv_gin_idx
    ON "dev"."user" USING GIN(user_display_name_tsv);
  `.execute(db);
}

export async function down(db: TKyselyDB): Promise<void> {
  await sql`DROP INDEX user_display_name_tsv_gin_idx`.execute(db);
